...
namespace KSP_PAPI
{
    public class KSP_PAPI : MonoBehaviour
    {
        ...
        public float CutoffAngle { get; set; } = 70f;
        ...
        void Update()
        {
            ...
            float camAngle = Vector3.Angle(Vector3.left, camPos);
            float angle = Mathf.Rad2Deg * Mathf.Asin(camPos.y / -camPos.x);
            float fade = (Mathf.Abs(camAngle) / CutoffAngle - 1);
